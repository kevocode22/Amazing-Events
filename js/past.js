...
function displayCard(data) {
  ...
  for (i = 0; i < data.length; i++) {
    if (fechaActual > data[i].date)
      cards += ` <div class="card raise mt-5 mb-5 bg-dark rounded" style="width: 20rem;">
      <img src="${data[i].image}" id="img-events" class="img-events card-img-top mt-3 pe-2 ps-2 bg-dark" style="height:30vh" alt="EventImage">
      <div class="card-body font bg-dark">
          ...
          <div class="d-flex justify-content-between align-items-center">
            <p class="fw-bold">Price: $${data[i].price}</p><a href="/details.html?id=${data[i].id}"
                class="btn justify-content-between bg-danger text-white">See more</a>
          </div>
      </div>
  </div>
      `;
